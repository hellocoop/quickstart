<script>
  ...
  $: selectedAppID,
    (() => {
      ...
      sendDarkImageUri =
        !!sessionStorage.dark_image_uri || !!_selectedAppData?.dark_image_uri
    })()
    ...
    try {
      ...
      if (response_uri) {
        ...
        $notification = {
          text:
            app_name +
            (selectedAppID === "create" ? " was created" : " was selected"),
          type: "success",
        }
      }
    }
...
<form on:submit|preventDefault={submit} class="mt-4">
        ...
        {#if selectedAppID === "create"}
          <div class="ml-7 space-y-0.5" transition:slide|local>
            {#if sessionStorage.tos_uri}
              <div>
                <label for="terms-of-service" class="text-sm opacity-60"
                ...
                <div class="flex items-center">
                  <input
                    ...
                    class="form-checkbox"
                  />
                  <a href={customTosUri} target="_blank" class="block text-sm break-all ml-2"
                    >{customTosUri}</a
                  >
                </div>
              </div>
            ...
            {#if sessionStorage.pp_uri}
              <div>
                <label for="privacy-policy" class="text-sm opacity-60"
                ...
                <div class="flex items-center">
                  <input
                    ...
                    class="form-checkbox"
                  />
                  <a href={customPpUri} target="_blank" class="block text-sm break-all ml-2"
                    >{customPpUri}</a
                  >
                </div>
              </div>
            ...
            {#if serverCanFetchLogo}
              <div>
                <label for="logo-light-mode" class="text-sm opacity-60"
                  >Logo (Light mode)</label
                >
                <div class="flex items-center">
                  <input
                  ...
                  />
                  <div
                    class="bg-white w-20 h-20 rounded-md inline-flex items-center justify-center ml-2 p-1.5 mt-0.5"
                  >
                    <img
                      src={sessionStorage.image_uri}
                      alt="{applicationName} light mode logo"
                    />
                  </div>
                </div>
              </div>
            ...
            {#if serverCanFetchDarkLogo}
              <div>
                <label for="logo-dark-mode" class="text-sm opacity-60"
                  >Logo (Dark mode)</label
                >
                <div class="flex items-center">
                  <input
                  ...
                  />
                  <div
                    class="bg-[#151515] w-20 h-20 rounded-md inline-flex items-center justify-center ml-2 p-1.5 mt-0.5"
                  >
                    <img
            ...
            {#if sessionStorage.redirect_uri}
              <div>
                <label for="redirect_uri" class="text-sm opacity-60"
                  >Redirect URI(s)</label
                >
                {#each sessionStorage.redirect_uri.split(" ") as redirect_uri, index}
                  <span class="block ml-6 text-sm break-all">{redirect_uri}</span>
                {/each}
              </div>
            {/if}
          </div>
  ...
  <button
    class="hello-btn-black-and-static w-full flex items-center justify-center rounded-md mt-4 h-11 border-2 border-[#808080] disabled:opacity-50"
    disabled={submitAjax ||
      (selectedAppID === "create" && !applicationName.length)}
